...
async function getArticles() {
  ...
  // Articles published by Sveltia in /stories
  try {
    const r = await fetch(
...
function card(a) {
  return `
    <a class="story-card" href="article.html?id=${encodeURIComponent(a.id)}">
      ...
      <div class="meta">${esc(a.category)} · ${esc(a.date)}</div>
      <h2>${esc(a.title)}</h2>
      <p>${esc(a.excerpt)}</p>
    </a>
  `;
...
(async () => {
  ...
  if (ct) {
    const n =
      new URLSearchParams(location.search).get("cat") || "All stories";
  ...
  if (at) {
    ...
    document.title = a.title + " — Becoming, by Stan";
    ...
    if (subtitle) subtitle.textContent = a.subtitle || "";
    if (meta) meta.textContent = a.date;
  ...
  el.innerHTML = d.length
    ? d.map(a => `
        <div class="draft">
          ...
          <button onclick="deleteDraft('${a.id}')">Delete</button>
        </div>
      `).join("")
